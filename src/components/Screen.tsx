...
const Screen = forwardRef<IFullScreenElement, IScreenProps>(
  ({ pauseHandler, fullScreenHandler }, ref) => {
    ...
    return (
      <div
        className={
          isFullScreen ? 'emulator__screen-wrapper fullscreen' : 'emulator__screen-wrapper'
        }
        ref={ref}
      >
        <canvas className="emulator__screen" id="game" width={256} height={240} />
        {!isStarted && gameRom && (
          <button className="emulator__start-button" type="button" onClick={startHandler}>
            Start Game
          </button>
        )}
        ...
        {isMobile && isFullScreen && (
          <button
            className="emulator__fs-button-close"
        ...
        {isMobile && isFullScreen && isStarted && (
          <button
            className="emulator__pause-button"
          ...
          >
            {!isPaused ? (
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                <g id="pause_circle" data-name="pause circle">
                  <path d="M12,0A12,12,0,1,0,24,12,12,12,0,0,0,12,0Zm0,22A10,10,0,1,1,22,12,10,10,0,0,1,12,22Z" />
                  <path d="M14,8v8a1,1,0,0,0,2,0V8A1,1,0,0,0,14,8Z" />
                  <path d="M8,8v8a1,1,0,0,0,2,0V8A1,1,0,0,0,8,8Z" />
                </g>
              </svg>
            ) : (
              <svg
